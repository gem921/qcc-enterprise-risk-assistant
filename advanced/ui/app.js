...
function setSession(session) {
  ...
  if (currentSession.riskPushStatus === "local_only" && reviewedSessionId !== currentSession.id) {
    reviewedSessionId = currentSession.id;
    elements.warningLevelFilter.value = "";
  ...
  buttons.continue.disabled = !running || !currentSession.awaitingContinue;
...
function renderRiskPushReview(session) {
  ...
  const failedCount = session.failedCompanyCount || 0;
  const publishableCount = session.publishableCompanyCount || 0;

  if (isScanResult && pushStatus === "awaiting_confirmation") {
    phase = failedCount ? "failed" : "pending";
    title = failedCount
      ? `扫描完成，${failedCount} 家未完整核验`
      : `待确认回传 ${session.riskPushCount || 0} 家重大风险公司`;
    message = failedCount
      ? publishableCount
        ? `仍有 ${publishableCount} 家明确结果可回传；失败或部分成功公司不会被当作无风险，也不会解除原风险。`
        : "本次没有可回传的完整结果，不能判定为无风险。请查看结果表中的错误信息后重新扫描。"
      : session.riskPushCount
    ...
    showButton = publishableCount > 0;
  } else if (isScanResult && pushStatus === "local_only") {
    phase = "success";
    title = `高级版扫描结果已保存，${session.riskPushCount || 0} 家存在近期风险`;
    message = `当前环境未启用结果回传接口，${session.publishableCompanyCount || 0} 家可确认结果仅保存在本地。`;
...
function renderScanProgress(session) {
  ...
  } else if (isScan && session.riskPushStatus === "local_only") {
    phase = "complete";
    percent = 100;
    label = "扫描完成，结果已保存本地";
    meta = `已保存 ${session.riskPushCount || 0} 家近期风险公司`;
  } else if (isScan && session.status === "done") {
    phase = session.riskPushStatus === "failed" || session.failedCompanyCount ? "warning" : "complete";
    percent = 100;
    label = session.riskPushStatus === "failed"
      ? "扫描完成，风险回传失败"
      : session.failedCompanyCount
        ? `扫描完成，${session.failedCompanyCount} 家未完整核验`
        : "扫描任务已完成";
...
function sessionMetaText(session) {
  if (session.riskPushStatus === "success") {
    return `风险结果已回传 ${session.riskPushCount || 0} 家有风险公司`;
  }
  if (session.riskPushStatus === "skipped") {
    return "本次扫描无重大风险";
  }
  if (session.riskPushStatus === "failed") {
    return session.riskPushError || "风险结果回传失败";
  }
  if (session.riskPushStatus === "pushing") {
    return `正在回传 ${session.riskPushCount || 0} 家有风险公司及正常状态`;
  }
  if (session.riskPushStatus === "awaiting_confirmation") {
    return session.failedCompanyCount
      ? `${session.failedCompanyCount} 家未完整核验，不能判定为无风险`
      : `待确认 ${session.riskPushCount || 0} 家有风险公司，确认后保存整批结果`;
  }
