...
const CLI_SCRIPT = join(SCRIPT_DIR, "qcc-risk-rpa.mjs");
...
const RULES_FILE = join(SCRIPT_DIR, "risk-rules.json");
...
用法:
  node qcc-risk-rpa-ui.mjs
  node qcc-risk-rpa-ui.mjs --port 18080
`);
}
...
async function finishSession(session, code, signal) {
  ...
  if (session.kind === "scan" && code === 0) {
    try {
      ...
      addLog("stderr", `重大风险审核数据准备失败：${session.riskPushError}`);
    }
  }
...
function prepareMajorRiskReviewForSession(session) {
  ...
  const payload = createMajorRiskBatch({
    rows: results.rows,
  ...
  session.riskPushCount = payload.records.length;
  ...
  addLog(
    ...
    payload.records.length
      ? `扫描结果待确认，共 ${payload.records.length} 家重大风险公司。`
      : "扫描结果待确认，本次无重大风险公司。",
  );
}
...
async function pushMajorRisksForSession(session) {
  ...
  if (payload.records.length) {
    addLog("system", `正在向${session.backendEnvironment.name}回传 ${payload.records.length} 家重大风险公司。`);
  } else {
    addLog("system", `正在向${session.backendEnvironment.name}回传扫描快照，本次无重大风险公司。`);
  }

  let lastError = null;
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      await pushMajorRiskBatch({
      ...
      addLog(
        "system",
        payload.records.length
          ? `重大风险回传成功，共 ${payload.records.length} 家。`
          : "扫描快照回传成功，本次无重大风险公司。",
      );
      if (activeSession === session) {
        broadcast("state", sessionSummary());
      ...
      lastError = error;
      addLog("stderr", `重大风险回传第 ${attempt} 次失败：${error.message || error}`);
      if (attempt < 3) {
  ...
  addLog("stderr", "扫描结果已保留，但重大风险回传失败，请根据推送状态文件排查后端连接。");
  if (activeSession === session) {
    broadcast("state", sessionSummary());
  }
}
